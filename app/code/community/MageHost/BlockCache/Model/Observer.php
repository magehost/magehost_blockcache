...
class MageHost_BlockCache_Model_Observer extends Mage_Core_Model_Abstract
{
    const CONFIG_SECTION = 'magehost_blockcache';
    ...
    function coreBlockAbstractToHtmlBefore( $observer ) {
        ...
        if ( $cacheKey ) {
            ...
            elseif ( 0 === strpos($cacheKey,'MH_') &&   // A block we manage caching for
                     $this->isCacheWarmer() &&
    ...
    public function controllerFrontInitBefore( /** @noinspection PhpUnusedParameterInspection */ $observer ) {
        if ( ! $this->isAdmin() ) {
            ...
            // If URL contains mhflush param, clean all blocks with the URL tag from the cache
     ...
     * Listens to event "mh_cache_save_block"
     *
     * @param Varien_Event_Observer $observer
     */
    public function cacheSave( $observer ) {
        ...
        if ( preg_match('|^'.$pregPrefix.'MH_PRD_|i',$id) ) {
